    ...
    def __getitem__(self, data_id):
        while True:
            ...
            if self.infer:
                ...
                data = {"text": text, "first_frame": first_frame, 'ref_video':ref_video, 'ref_path': ref_path, 'content_video': content_video, 'content_path': content_path}
                break
            else:
                text = self.text[data_id]
...
def parse_args():
    parser = argparse.ArgumentParser(description="CamCloneMaster Inference")
    parser.add_argument(
        "--dataset_path",
        type=str,
    ...
    parser.add_argument(
        "--ckpt_path",
        type=str,
        default="models/CamCloneMaster-Wan2.1/CamCloneMaster-Step9500.ckpt",
        help="Path to save the model.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="demo/camclone_i2v_output",
...
if __name__ == '__main__':
    ...
    # 3. Load CamCloneMaster checkpoint
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        content_video = batch['content_video']
        ...
        save_video(video, os.path.join(output_dir, f"camclone_output_video_{batch_idx:03d}.mp4"), fps=30, quality=5)