...
def pad_latents_to_channels(latents, target_channels):
    ...
    if pad_channels < 0:
        raise ValueError(
            f"Reference latent channels ({latents.shape[1]}) exceed target channels ({target_channels})."
        )
...
class WanVideoI2VPipeline(BasePipeline):
    ...
    def __call__(
        ...
        if input_image is not None and self.image_encoder is not None:
            ...
            image_emb = {}
        target_channels = noise.shape[1] + (image_emb["y"].shape[1] if "y" in image_emb else 0)
        ...
        if ref_video is not None:
            ...
            ref_latents = pad_latents_to_channels(ref_latents, target_channels)
        else:
            ref_latents = None
        ...
        if content_video is not None:   
            ...
            content_latents = pad_latents_to_channels(content_latents, target_channels)
        else:
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # Scheduler
            latents = self.scheduler.step(noise_pred.cpu()[::, ::, :latents.shape[2]], self.scheduler.timesteps[progress_id], latents_input.cpu()).cuda()
