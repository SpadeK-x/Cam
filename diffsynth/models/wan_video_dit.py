...
def pad_latents_to_channels(latents: Optional[torch.Tensor], target_channels: int):
    if latents is None:
        return None
...
class WanModel(torch.nn.Module):
    ...
    def forward(self,
        ...
        if ref_latents is not None and content_latents is not None:
            ref_latents = pad_latents_to_channels(ref_latents.to(x.device), x.shape[1])
            content_latents = pad_latents_to_channels(content_latents.to(x.device), x.shape[1])
